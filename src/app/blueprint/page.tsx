...
import { ArrowLeft, Save, Maximize2, MousePointer2, ZoomIn, ZoomOut, RotateCcw } from 'lucide-react';
...
export default function BlueprintMapPage() {
    ...
    return (
        <div className="min-h-screen p-4 flex flex-col no-scrollbar">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    {!isEditMode && (
                        <div className="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-900 rounded-xl p-1 border border-zinc-200 dark:border-zinc-800">
                            <button onClick={() => setZoom(z => clampZoom(z - 0.2))} className="p-1.5 hover:bg-white dark:hover:bg-zinc-800 rounded-lg transition-colors">
                            ...
                            <button onClick={resetView} className="p-1.5 hover:bg-white dark:hover:bg-zinc-800 rounded-lg transition-colors">
            ...
            {/* Canvas Area - with zoom/pan wrapper */}
            <div
                ref={canvasRef}
                className={`flex-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl relative overflow-hidden shadow-inner ${isEditMode ? '' : 'cursor-grab active:cursor-grabbing'}`}
                onWheel={handleCanvasWheel}
                onPointerDown={handleCanvasPointerDown}
                onPointerMove={handleCanvasPointerMove}
                onPointerUp={handleCanvasPointerUp}
                onDoubleClick={!isEditMode ? resetView : undefined}
                style={{ touchAction: isEditMode ? 'none' : 'none' }}
            >
                {/* Grid Canvas */}
                ...
                >
                    {/* Nodes — pointer-events re-enabled per-node when not editing */}
